...
def save_results(results, seeds, outdir):
    """Write the summary as both a CSV (machine-readable) and a markdown table
    (paste-ready for the README). Regenerated on every run, so the repo's record
    of the result is produced by the code — not hand-transcribed."""
    os.makedirs(outdir, exist_ok=True)
    multi = seeds > 1

    # build rows of plain values
    header = ["dataset", "predictor_mutual_R2", "black_box_R2", "var_2d",
              "coverage@10", "fidelity@10", "fidelity@40"]
    if multi:
        header += ["coverage@10_std", "fidelity@10_std"]
    rows = []
    for r in results:
        c10 = r["rows"][10]; c40 = r["rows"][40]
        if multi:
            # (covM, covS, fidM, fidS, spM, spS)
            rows.append([r["label"], r["setR2"], r["rf_r2"], r["var2d"],
                         c10[0], c10[2], c40[2], c10[1], c10[3]])
        else:
            # (cov, fid, sp)
            rows.append([r["label"], r["setR2"], r["rf_r2"], r["var2d"],
                         c10[0], c10[1], c40[1]])
    ...
    # CSV
    csv_path = os.path.join(outdir, "decoupling_results.csv")
    df = pd.DataFrame(rows, columns=header)
    df.to_csv(csv_path, index=False)

    # markdown
    md_path = os.path.join(outdir, "decoupling_results.md")
    seedtag = f"mean of {seeds} seeds" if multi else "single seed"
    with open(md_path, "w") as f:
        f.write(f"# Decoupling across real datasets ({seedtag})\n\n")
        f.write("Generated by `experiments/decoupling_multidataset.py`. "
                "Coverage stays high/flat across datasets while fidelity error "
                "rises as redundancy falls — the two axes are decoupled.\n\n")
        if multi:
            f.write("| dataset | mutual R² | black-box R² | coverage@10 | fidelity@10 | fidelity@40 |\n")
            f.write("|---|--:|--:|--:|--:|--:|\n")
            for r in results:
                c10 = r["rows"][10]; c40 = r["rows"][40]
                f.write(f"| {r['label']} | {r['setR2']:.3f} | {r['rf_r2']:.3f} | "
                        f"{c10[0]:.1%} ±{c10[1]:.1%} | {c10[2]:.4f} ±{c10[3]:.4f} | "
                        f"{c40[2]:.4f} ±{c40[3]:.4f} |\n")
        else:
            f.write("| dataset | mutual R² | black-box R² | coverage@10 | fidelity@10 | fidelity@40 |\n")
            f.write("|---|--:|--:|--:|--:|--:|\n")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--outdir", default=os.path.join(HERE, "..", "results"),
                    help="where to write decoupling_results.{csv,md}")
    args = ap.parse_args()
    ...
    if len(results) >= 2:
        ...
        save_results(results, args.seeds, args.outdir)